...
function panelPricing(p, t, f) {
  ...
  if (t === "short") {
    ...
    html += "<li><span class='badge b-amber'>易混淆</span> <b>定价预定利率 ≠ 现金价值计算用利率</b>：计算保单最低现金价值时，须在报备时厘定保险费所采用的预定利率基础上<b>再加法定加点</b>——分红型保险期间10年及以下 +1.0%、10年以上 +1.5%（保监发〔2015〕93号第九条）；传统型 +2%（银保监办发〔2020〕7号第九条）。详见<b>面板5「现金价值计量设计 · 现金价值计算用利率」</b>。</li>";
  }
  html += "</ul>";
  ...
  html += "<li>官方上限数据（普通型 + 分红型对照）、现价计算分年度费用率表、k 值表：<a href='baoxing.html#fees'>报行合一费用管理全解 · 第一节</a>（双向入口）。</li>";
  html += "</ul>";
...
/* 现金价值计算用利率表（分红型 93号 / 普通型 7号 / 万能型 19号） */
function cvRateTable(f) {
  var cur = function (k) { return f === k ? " <span class='badge b-green'>当前组合</span>" : ""; };
  var html = "<h4>② 现金价值计算用利率（预定利率的选取）</h4>";
  html += "<table class='tbl kv'><tr><th>险种</th><th>保险期间</th><th>现金价值计算用利率</th></tr>";
  html += "<tr><td><b>分红型</b></td><td>10年及以下</td><td><b>预定利率 + 1.0%</b>　示例：1.75% → <b>2.75%</b>" + cur("dividend") + "</td></tr>";
  html += "<tr><td><b>分红型</b></td><td>10年以上</td><td><b>预定利率 + 1.5%</b>　示例：1.75% → <b>3.25%</b>" + cur("dividend") + "</td></tr>";
  html += "<tr><td><b>传统型（普通型）</b></td><td>一年以上</td><td><b>险种报备时厘定保险费所使用的预定利率 + 2%</b>　示例：2.0% → <b>4.0%</b>" + cur("traditional") + "</td></tr>";
  html += "<tr><td>万能型</td><td>—</td><td>不适用本表：按<b>保单账户价值 − 退保费用</b>确定，另受最低保证利率约束" + cur("universal") + "</td></tr>";
  html += "</table>";
  html += "<ul><li>表中「预定利率」指本产品报备时厘定保险费所采用的预定利率；示例数值按现行上限（普通型2.0%／分红型1.75%）试算，<b>实际以本产品报备利率为准</b>。</li>";
  html += "<li>注意区分两套利率：①<b>定价预定利率</b>（面板3，受上限约束）；②<b>现金价值计算用利率</b>（本表，= 定价预定利率 + 法定加点，仅用于算最低现金价值，<b>加点不是定价利率</b>）。</li></ul>";
  html += "<p class='cite'>依据：分红型 <b>保监发〔2015〕93号</b>附件《分红保险精算规定》第九条（利率表：10年及以下 +1.0%、10年以上 +1.5%）；传统型 <b>银保监办发〔2020〕7号</b>《普通型人身保险精算规定》第九条（预定利率 +2%）；万能型 <b>保监发〔2015〕19号</b>《万能保险精算规定》。</p>";
  return html;
}

/* 长期险现金价值计量链条：PVR → MCV → 保单现金价值 */
function cvChain(p, t, f) {
  var html = "<h4>① 保单年度末保单价值准备金 PVR（最低现金价值的计算基数）</h4><ul>";
  html += "<li>定义：为计算保单年度末保单最低现金价值，按法定计算基础与方法算得的准备金数值；<b>不包括该保单年度末的生存给付金额</b>。</li>";
  html += "<li>计算基础三项：①<b>发生率</b>——采用险种报备时厘定保险费所使用的预定发生率；②<b>附加费用率</b>——采用法定表定数值（<b>非公司实际费用</b>，见<a href='baoxing.html#fees'>报行合一专页</a>表3/表7）；③<b>利率</b>——按下方利率表选取。</li>";
  html += "<li>计算方法：按该保单的保险责任与各保单年度<b>净保费</b>（= 按 PVR 计算基础重新计算的毛保费 ×(1−法定费用率)），采用<b>未来法</b>计算。</li>";
  html += "</ul>";
  html += cvRateTable(f);
  html += "<h4>③ 保单年度末保单最低现金价值 MCV</h4>";
  html += "<table class='tbl kv'><tr><th>项目</th><th>口径</th></tr>";
  html += "<tr><td>公式</td><td><b>MCV = r × max(PVR, 0)</b></td></tr>";
  html += "<tr><td>系数 r</td><td>r = k + t×(1−k)／min(20,n)，当 t &lt; min(20,n)；t ≥ min(20,n) 时 <b>r = 1</b><br>n = 交费期间（趸交 n = 1）；t = 已经过保单年度</td></tr>";
  html += "<tr><td>参数 k</td><td>" + (f === "dividend"
    ? "两全保险／年金保险：期交个人 <b>0.9</b>、期交团体 0.95、趸交 1；终身寿险：期交个人 <b>0.8</b>、期交团体 0.85、趸交 1（保监发〔2015〕93号）"
    : "年金保险：期交个人 <b>0.9</b>、期交团体 0.95、趸交 1；两全保险 <b>0.85</b>；定期寿险／终身寿险／健康险／意外险：期交个人 <b>0.8</b>、期交团体 0.85、趸交 1（银保监办发〔2020〕7号）")
    + " —— 完整表见<a href='baoxing.html#fees'>" + (f === "dividend" ? "表8（分红型）" : "表4（普通型）") + "</a></td></tr>";
  html += "</table>";
  html += "<h4>④ 保单年度末／年度中保单现金价值</h4><ul>";
  html += "<li>公司可直接以 MCV 作为保单年度末保单现金价值，也可按其他合理计算基础与方法确定，<b>但不得低于 MCV</b>；精算报告中须明确体现该比较（<a href='negative-list.html'>负面清单</a>第62条）。</li>";
  html += "<li>保单年度中现金价值：根据保单年度末现金价值按合理方法确定（本站<a href='formula-cv.html'>现价公式专页</a>的年中口径）。</li>";
  html += "<li>采用增额红利分配方式的分红险：除基本保额现金价值外，<b>还需计算每单位增额红利现金价值</b>，同样不得低于按上述基础与方法算得的结果。</li>";
  html += "</ul>";
  return html;
}
...
function panelReserve(p, t, f) {
  var m = PMETA[p];
  var html = "<h4>现金价值计量设计</h4><ul>";
  if (t === "short") {
    html += "<li>短期险通常无现金价值或现金价值极低（未满期净保费口径）；若含满期给付/返还性质，须按未满期责任处理并做现金流匹配。</li>";
    html += "<li>保险期间一年及以内：按<b>未经过净保费</b>口径，<b>保单年度中保单最低现金价值不得低于未经过净保费</b>（银保监办发〔2020〕7号第十三条；<a href='negative-list.html'>负面清单</a>第63条）。</li>";
  } else if (f === "universal") {
    html += "<li>万能型不适用 PVR→MCV 链条：保单现金价值 = <b>保单账户价值 − 退保费用</b>，退保费用比例须在条款中载明并逐年递减；最低保证利率另受现行上限约束（保监发〔2015〕19号，现行上限1.0%）。</li>";
    html += "<li>现金价值走势是产品「储蓄化」程度的直接信号：过早接近或超过已交保费将触发高现价/中短存续期监管逻辑（历史教训见保监发〔2016〕22号/76号体系）。</li>";
  } else {
    html += "<li>现金价值不是公司可以随意定的金额：其<b>法定下限由「保单年度末保单最低现金价值 MCV」锁死</b>，计算链条见下方①→④。</li>";
    html += "<li>现金价值走势是产品「储蓄化」程度的直接信号：过早接近或超过已交保费将触发高现价/中短存续期监管逻辑（历史教训见保监发〔2016〕22号/76号体系）。</li>";
    html += "<li>保证现金价值表须随条款报备并在犹豫期内提供给客户。</li>";
  }
  html += "</ul>";
  if (t === "long" && f !== "universal") html += cvChain(p, t, f);
  ...
  html += "<p class='cite'>主要依据（现金价值计量）：银保监办发〔2020〕7号《普通型人身保险精算规定》第九～十二条、保监发〔2015〕93号《分红保险精算规定》第九～十二条、保监发〔2015〕19号；准备金评估：银保监办发〔2019〕182号、保监发〔2016〕76号、金规〔2025〕21号。公式与演算见<a href='formula-cv.html'>现价公式专页</a>与<a href='pricing-lab.html'>保险产品精算AI助手</a>。</p>";
  return html;
}
